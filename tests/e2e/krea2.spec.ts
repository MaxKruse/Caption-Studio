...
test.describe("Krea 2 Mode", () => {
  ...
  test("configure phase exposes a max image dimension override", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Continue (1 image)" }).click();

    const input = page.getByLabel("Max image dimension (px)");
