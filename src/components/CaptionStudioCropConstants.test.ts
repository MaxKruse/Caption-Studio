...
describe("getDetectionPrompts — SFW", () => {
  it("returns both systemPrompt and userPrompt", () => {
    const prompts = getDetectionPrompts("sfw", "qwen-vl-max");
  ...
  it("systemPrompt instructs to detect faces and bodies", () => {
    const { systemPrompt } = getDetectionPrompts("sfw", "qwen-vl-max");
  ...
  it("SFW userPrompt instructs to detect ALL faces and bodies", () => {
    const { userPrompt } = getDetectionPrompts("sfw", "qwen-vl-max");
...
describe("getDetectionPrompts — NSFW", () => {
  it("returns both systemPrompt and userPrompt", () => {
    const prompts = getDetectionPrompts("nsfw", "qwen-vl-max");
    expect(prompts).toHaveProperty("systemPrompt");
    expect(prompts).toHaveProperty("userPrompt");
  });

  it("NSFW systemPrompt is identical to SFW systemPrompt", () => {
    const { systemPrompt: sfwSystem } = getDetectionPrompts("sfw", "qwen-vl-max");
    const { systemPrompt: nsfwSystem } = getDetectionPrompts("nsfw", "qwen-vl-max");
    expect(nsfwSystem).toBe(sfwSystem);
  });

  it("NSFW userPrompt instructs to detect ALL faces and bodies", () => {
    const { userPrompt } = getDetectionPrompts("nsfw", "qwen-vl-max");
    expect(userPrompt).toContain("Detect ALL faces");
    expect(userPrompt).toContain("ALL bodies");
  });
  ...
  it("NSFW userPrompt is identical to SFW userPrompt (no mode-specific scoring)", () => {
    ...
    const { userPrompt: sfw } = getDetectionPrompts("sfw", "qwen-vl-max");
    const { userPrompt: nsfw } = getDetectionPrompts("nsfw", "qwen-vl-max");
...
describe("getDetectionPrompts — common", () => {
  ...
  it("systemPrompt is consistent across calls", () => {
    const { systemPrompt: a } = getDetectionPrompts("sfw", "qwen-vl-max");
    const { systemPrompt: b } = getDetectionPrompts("sfw", "qwen-vl-max");
    expect(a).toBe(b);
  });

  it("total prompt length is reasonable (under 1000 chars combined)", () => {
    const sfw = getDetectionPrompts("sfw", "qwen-vl-max");
    const sfwTotal = sfw.systemPrompt.length + sfw.userPrompt.length;
    expect(sfwTotal).toBeLessThan(1000);

    const nsfw = getDetectionPrompts("nsfw", "qwen-vl-max");
...
// ---------------------------------------------------------------------------
// getDetectionPrompts — Gemma model (box_2d, y-first)
// ---------------------------------------------------------------------------

describe("getDetectionPrompts — Gemma model", () => {
  it("Gemma model uses box_2d (y-first) format", () => {
    const { systemPrompt } = getDetectionPrompts("sfw", "gemma-3-12b-it");
    expect(systemPrompt).toContain("box_2d");
    expect(systemPrompt).toContain("y_min, x_min, y_max, x_max");
    expect(systemPrompt).not.toContain("bbox_2d");
  });

  it("Gemma model with mixed case is detected", () => {
    const { systemPrompt } = getDetectionPrompts("sfw", "Gemma-3-27b-it");
    expect(systemPrompt).toContain("box_2d");
  });

  it("non-Gemma model uses bbox_2d (x-first) format", () => {
    const { systemPrompt } = getDetectionPrompts("sfw", "qwen-vl-max");
    expect(systemPrompt).toContain("bbox_2d");
    expect(systemPrompt).toContain("x_min, y_min, x_max, y_max");
    // bbox_2d contains box_2d as substring, so check for the single-underscore variant alone
    expect(systemPrompt).not.toMatch(/[^b]box_2d/);
  });

  it("Gemma SFW and NSFW share the same systemPrompt", () => {
    const { systemPrompt: sfw } = getDetectionPrompts("sfw", "gemma-3-12b-it");
    const { systemPrompt: nsfw } = getDetectionPrompts("nsfw", "gemma-3-12b-it");
    expect(sfw).toBe(nsfw);
  });

  it("Gemma userPrompt is identical to SFW userPrompt", () => {
    const { userPrompt } = getDetectionPrompts("sfw", "gemma-3-12b-it");
    expect(userPrompt).toContain("Detect ALL faces");
  });

  it("Gemma NSFW userPrompt is identical to SFW userPrompt", () => {
    const { userPrompt: sfw } = getDetectionPrompts("sfw", "gemma-3-12b-it");
    const { userPrompt: nsfw } = getDetectionPrompts("nsfw", "gemma-3-12b-it");
    expect(sfw).toBe(nsfw);
  });

  it("Qwen and Gemma systemPrompts differ (different coord formats)", () => {
    const { systemPrompt: qwen } = getDetectionPrompts("sfw", "qwen-vl-max");
    const { systemPrompt: gemma } = getDetectionPrompts("sfw", "gemma-3-12b-it");
    expect(qwen).not.toBe(gemma);
    expect(qwen).toContain("bbox_2d");
    expect(gemma).toContain("box_2d");
  });
});
